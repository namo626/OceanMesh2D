...
% Jan/2020 - handling for any geomagnetic coord system
...
if nargin==0 || ischar(X)
  ...
  if ~( strcmp(MAP_COORDS.name,MAP_PROJECTION.coordsystem.name))
    ...
    if strcmp(MAP_COORDS.name.name,'geographic')
      [long,lat]=mc_coords('XYmag2geo',long,lat);
    else
      [long,lat]=mc_coords('XYgeo2mag',long,lat);      
    end 
  end
end
