...
%   if strcmp(MAP_COORDS.name,MAP_PROJECTION.coordsystem.name) && ...  % using same coord system as projection
...
%      [X,Y,I]=feval(MAP_PROJECTION.routine,'ll2xy',varargin{:},'clip','on');
  ...
  if strcmp(MAP_COORDS.name,'geographic')    % Projection uses geomagnetic coords, data uses geographic
